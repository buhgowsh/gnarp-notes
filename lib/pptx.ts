...
// The PptxGenJS constructor is attached to the window object by the script
declare global {
  interface Window {
    PptxGenJS: any; 
  }
...
export const createPowerPoint = async (cards: Flashcard[], title: string): Promise<void> => {
  ...
  cards.forEach(card => {
    ...
    qSlide.addText(card.question, {
      x: 0.5, y: 1.0, w: '90%', h: '70%', fontFace: "Chakra Petch", fontSize: 36, color: "FFFFFF", align: 'center', valign: 'middle'
    });
    const aSlide = pptx.addSlide({ masterName: "GNARP_ANSWER" });
    aSlide.addText(card.answer, {
      x: 0.5, y: 1.0, w: '90%', h: '70%', fontFace: "Chakra Petch", fontSize: 32, color: "FFFFFF", align: 'center', valign: 'middle'
    });
  });
